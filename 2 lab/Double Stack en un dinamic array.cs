...
class TwoStack
{
    ...
    public void Push(StackNumber stackNumber, int value)
    {
        ...
        if (StackNumber.ONE == stackNumber)
        {
             
            if( size1== 0 && size2 == 0)
            {
                Resize(INITIAL_CAPACITY * arr.Length);
            }
            arr[size1] = value;
            size1++;
            if (size1 < arr.Length / 4  && size2 < arr.Length / 4 )
            {
                Resize(arr.Length / 2);
            }

        }
        if (StackNumber.TWO == stackNumber) 
        {
            if(size2 == 0)
            {
        ...
        if (size2 == arr.Length / 2 - 1 || size1 == arr.Length / 2 - 1)
        {
            Resize(arr.Length * INITIAL_CAPACITY);
    ...
    public int Pop(StackNumber stackNumber)
    {
        ...
        {
            if (size1 == 0)
                throw new Exception("NO hay valor que tirar");
            ret =  arr[size1 - 1];
            size1--;
    ...
    private void Resize(int newCapacity)
    {
        ...
        for (int i = arr.Length - 1, j = tmp.Length - 1; j > tmp.Length/2 - 1; i--, j--)
        {
            tmp[j] = arr[i];
        }
        // TODO: implementar
